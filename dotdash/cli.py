...
if not DOTFILES:
    epilog = click.style("No dotfiles directory initialized, use dotdash init", fg="red")
    DOTFILES_FULL = None
else:
...
if DOTFILES_FULL and DOTFILES_FULL.exists():
    epilog = click.style(f"Dotfiles directory is {DOTFILES}", fg="green")
elif DOTFILES_FULL:
    epilog = click.style(f"Configured directory {DOTFILES} does not exist", fg="red")
